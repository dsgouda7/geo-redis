...
fn station_to_aircraft(mut e: GeoEntry) -> GeoEntry {
    ...
    let condition_label = {
        let base = format!(
            "{} {}",
            open_meteo::wmo_emoji(wmo),
            open_meteo::wmo_label(wmo)
        );
        if flt_cat.is_empty() {
            base
        } else {
...
pub async fn all_stations(
    ...
    if s2 == RAW_LEVEL {
        ...
        let entries: Vec<GeoEntry> = raw
            .iter()
            .step_by(step)
            .map(|s| station_to_aircraft(raw_to_geo_entry(s)))
            .collect();
        return Json(AircraftResponse {
            count: entries.len(),
            aircraft: entries,
        });
    }
    ...
    if let Some(clusters) = cache.get(&s2) {
        let entries: Vec<GeoEntry> = clusters
            .iter()
