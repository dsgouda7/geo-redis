...
function DayNightBoundary({ coords, nightSide }: { coords: Array<{ lat: number; lon: number }>; nightSide: string }) {
  ...
  // Close the curve toward the pole on the night side to shade the dark region.
...
function SatelliteMarker({ sat, zoom }: { sat: Satellite; zoom: number }) {
  ...
  return (
    <CircleMarker center={[sat.lat, sat.lon]} radius={4} pathOptions={{ fillColor: color, fillOpacity: 0.85, color: '#000', weight: 1.5 }}>
